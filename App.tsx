...
const HeaderCard: React.FC<{ icon: React.ReactNode; label: string; value: string; onClick?: () => void }> = ({ icon, label, value, onClick }) => (
  <div
    onClick={onClick}
    role={onClick ? 'button' : undefined}
    tabIndex={onClick ? 0 : undefined}
    onKeyDown={onClick ? (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onClick(); } } : undefined}
    className={`bg-slate-800/60 border border-slate-700 p-4 sm:p-5 rounded-2xl flex items-center gap-4 hover:border-slate-500 transition-all hover:-translate-y-1 shadow-xl shadow-black/10 group min-w-0 h-full overflow-hidden ${onClick ? 'cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500' : ''}`}
  >
...
const App: React.FC = () => {
  ...
  const scrollToSection = (n: number) => {
    document.getElementById(`forensic-sec-${n}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };

  const StatsGrid = () => {
    const jump = result ? scrollToSection : undefined;
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8 w-full animate-fade-in">
        <HeaderCard icon={<Box className="text-blue-400 w-5 h-5" />} label="Infrastructure" value="Cloud & DNS Stack" onClick={jump && (() => jump(4))} />
        <HeaderCard icon={<ShieldAlert className="text-red-400 w-5 h-5" />} label="Compliance" value="Security Hardening" onClick={jump && (() => jump(6))} />
        <HeaderCard icon={<MousePointer2 className="text-green-400 w-5 h-5" />} label="Analytics" value="Behavioral Tracking" onClick={jump && (() => jump(5))} />
        <HeaderCard icon={<Tag className="text-yellow-400 w-5 h-5" />} label="Platform" value="CMS & Theme Tech" onClick={jump && (() => jump(1))} />
      </div>
    );
  };
      ...
      <main className="w-full max-w-4xl space-y-10">
        ...
        {result && (
          <div className="space-y-12">
            <ErrorBoundary fallbackLabel="Could not render this report">
